			...
			<?php
				...
				if($_SESSION['show']=="yes")
				{
					while($row = mysqli_fetch_array($result))
					{
						...
						if($row1[$pb_id.'_co']!=-1)
							echo "<td><a style='color:#555;background:#AAE2AB no-repeat center/32px url(check.svg);' href='./admin_compile_arena?contestid=$pb_id' class='btn btn-default'>Submit</a></td>";
						else if($row1[$pb_id.'_wr']!=0)
							echo "<td><a style='color:#898992;background:#FFE3E3' href='./admin_compile_arena?contestid=$pb_id' class='btn btn-default'>Submit</a></td>";
						else
							echo "<td><a style='color:#555' href='./admin_compile_arena?contestid=$pb_id' class='btn btn-default'>Submit</a></td>";
						echo "</tr>";
					}
				}
				mysqli_close($con);
