<?php
	...
	if($_SESSION['email']=='codeit@vidyanikethan.asia')
		header("location: admin_practice");
?>
<!DOCTYPE html>
<html>
	<head>
		...
		<div id="section" style="height: 1700px;">
			...
			<?php
				...
				while($row = mysqli_fetch_array($result))
				{
					...
					if($row1['score']>0)
						echo "<td><a style='color:#555;background:#AAE2AB no-repeat center/32px url(check.svg);' href='./compile_arena?practiceid=$pb_id' class='btn btn-default'>Submit</a></td>";
					else if($count!=0)
						echo "<td><a style='color:#898992;background:#FFE3E3' href='./compile_arena?practiceid=$pb_id' class='btn btn-default'>Submit</a></td>";
					else
						echo "<td><a style='color:#898992' href='./compile_arena?practiceid=$pb_id' class='btn btn-default'>Submit</a></td>";
					echo "</tr>";
				}
				echo "</table>";
				echo "<br><br>";
				...
				{
					$pre = $page - 1;
					echo "<a id='pre' href='/practice?page=$pre' class='btn btn-default' style='height:36px;line-height:36px;border:1px solid #E7E7E8;float:left;cursor:pointer;color:#898992;' role='button'>Prev</a>";
				}
				$offset = ($page+1)*3;
				$result = mysqli_query($con,"SELECT * FROM practice ORDER BY dlevel ASC, pb_id ASC LIMIT $limit OFFSET $offset");
				...
				{
					$next = $page + 1;
					echo "<a id='next' href='/practice?page=$next' class='btn btn-default' style='height:36px;line-height:36px;border:1px solid #E7E7E8;float:right;cursor:pointer;color:#898992;' role='button'>Next</a>";
				}
			?>
			<!---->
		</div>
